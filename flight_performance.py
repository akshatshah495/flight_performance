...
# Reading data and creating required functions
...
# Application layout (DIV0)
app.layout = html.Div(children=[
                                html.H1('US Domestic Airline Flights Performance',style={'textAlign':'center','color':'#503D36','font-size':24}),
    
                                # Dropdown creation
                                # Creating an outer division DIV1
                                html.Div([
                                         
                                          # Adding an division DIV2
                                          html.Div([
                                                    # Creating an division for adding dropdown helper text for report type DIV3
                                          ...
                                          # Adding next division DIV4
                                          html.Div([
                                                    # Creating an division for adding dropdown helper text for choosing year DIV5
                                                    html.Div([html.H2('Choose Year:', style={'margin-right': '2em'})]),
                                                    # dropdown 2
                                                    dcc.Dropdown(id='input-year',
                                                                 options=[{'label': i, 'value': i} for i in year_list],
                                                                 placeholder="Select a year", style={'width':'80%', 'padding':'3px', 'font-size': '20px', 'text-align-last' : 'center'}),
                                                   ], style={'display': 'flex'}),  
                                ...
                                # DIV7
                                html.Div([
                                          html.Div([ ], id='plot2'), # DIV8
                                          html.Div([ ], id='plot3')  # DIV9
                                         ], style={'display': 'flex'}),
                                # DIV10
                                html.Div([
                                          html.Div([ ], id='plot4'), # DIV11
                                          html.Div([ ], id='plot5')  # DIV12
                                         ], style={'display': 'flex'}),
...
def get_graph(chart, year, children1, children2, c3, c4, c5):
        ...
        if chart == 'OPT1':
            ...
            bar_fig = px.bar(bar_data, x='Month', y='Flights', color='CancellationCode', title='Monthly Flight Cancellation as per Cancellation Categories')            
            # Average flight time by reporting airline
            line_fig = px.line(line_data, x='Month', y='AirTime', color='Reporting_Airline', title='Average monthly flight time (minutes) as per Reporting Airlines')            
            ...
            map_fig = px.choropleth(map_data,  # Input data
                    ...
                    locationmode = 'USA-states', # Set plot as US States
